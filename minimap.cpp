...
Collision_Rect<double> Minimap::get_box() const{
    return Collision_Rect<double>((double)Game_Window::width()/2.0-((double)width-Game_Constants::MINIMAP_BORDER_THICKNESS)/2.0,
                                  (double)Game_Window::height()/2.0-((double)height-Game_Constants::MINIMAP_BORDER_THICKNESS)/2.0,
    ...
    if(is_generated()){
        ...
        if(Game::player_has_contract()){
            ...
            if(!Game::is_object_over_planet(vertices,planet)){
                Coords<double> planet_position(planet.get_circle().x/get_scale_x(),planet.get_circle().y/get_scale_y());

                Render::render_line(box.x+player_x,box.y+player_y,box.x+planet_position.x,box.y+planet_position.y,1.0,"minimap_arrow");

                double angle=planet.get_circle().get_angle_to_rect(player.get_box());

                Vector draw_vertex_1(Game_Constants::MINIMAP_ARROW_MAGNITUDE,angle-Game_Constants::MINIMAP_ARROW_ANGLE_MOD);
                Vector draw_vertex_2(Game_Constants::MINIMAP_ARROW_MAGNITUDE,angle+Game_Constants::MINIMAP_ARROW_ANGLE_MOD);
                ...
                Render::render_line(box.x+planet_position.x,box.y+planet_position.y,box.x+vertex_1.x,box.y+vertex_1.y,1.0,"minimap_arrow");
                Render::render_line(box.x+planet_position.x,box.y+planet_position.y,box.x+vertex_2.x,box.y+vertex_2.y,1.0,"minimap_arrow");
                Render::render_line(box.x+vertex_1.x,box.y+vertex_1.y,box.x+vertex_2.x,box.y+vertex_2.y,1.0,"minimap_arrow");
            }
        }
        ...
        player_sprite.render(box.x+player_x-player.get_box().w*scale_x/2.0,box.y+player_y-player.get_box().h*scale_y/2.0,1.0,scale_x,scale_y,player.get_angle());
    }
}
