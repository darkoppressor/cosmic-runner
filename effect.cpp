...
Effect::Effect (std::string new_sprite, bool new_fade, double new_scale, const Coords<double>& position, string sound,
                const Vector& new_velocity, double new_angle, const Vector& new_angular_velocity, uint32_t seconds,
                bool new_line, const Coords<double>& end_point, string new_color, string new_text, bool new_cloud) {
    done = false;
    opacity = 1.0;
    fade = new_fade;
        ...
        if (text.length() > 0) {
            if (Collision::check_rect(box * Game_Manager::camera_zoom, Game_Manager::camera)) {
                ...
                font->show(box.x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                           box.y * Game_Manager::camera_zoom - Game_Manager::camera.y, text, color, opacity, scale,
                           scale, angle);
            }
        } else if (!line) {
            ...
            if (Collision::check_rect(box_for_camera_check * Game_Manager::camera_zoom, Game_Manager::camera)) {
                ...
                sprite.render(box.x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                              box.y * Game_Manager::camera_zoom - Game_Manager::camera.y, opacity, scale, scale, angle,
                              color);
            }
            ...
            Render::render_line(box.x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                ...
                                box.h * Game_Manager::camera_zoom - Game_Manager::camera.y, opacity, color);
        }
    }
}
