...
vector<Coords<double>> Ship::get_collision_vertices () const {
    ...
    double translate_x = vertices_collision[Collision::VERTEX_UPPER_LEFT].x -
                         vertices_box[Collision::VERTEX_UPPER_LEFT].x;
    double translate_y = vertices_collision[Collision::VERTEX_UPPER_LEFT].y -
                         vertices_box[Collision::VERTEX_UPPER_LEFT].y;
    ...
    if (is_alive()) {
        ...
        if (effective_damage > 0) {
            ...
            if (is_player) {
                ...
                Controller_Manager::make_rumble(Controller_Manager::CONTROLLER_ID_ALL,
                                                Game_Constants::SHAKE_MAGNITUDE_DAMAGE,
                                                Game_Constants::SHAKE_LENGTH_DAMAGE);
            }
...
void Ship::regenerate_shields (bool is_player) {
    if (is_alive() && !is_disabled(is_player) && is_in_processing_range()) {
        if (shields < get_shields_max() &&
            ++shield_recharge >= get_shield_recharge_rate() * Engine::UPDATE_RATE / 1000) {
            shield_recharge = 0;
...
void Ship::cooldown (const Quadtree<double, uint32_t>& quadtree_ships, const Quadtree<double, uint32_t>& quadtree_shots,
                     RNG& rng, uint32_t own_index) {
    bool is_player = own_index == 0;
    ...
    if (is_alive() && is_in_processing_range()) {
        ...
        if (has_weapon()) {
            uint32_t cool_point = get_cooldown(Game_Data::get_upgrade_type(get_weapon_name())->cooldown) *
                                  Engine::UPDATE_RATE / 1000;

            if (weapon_cooldown < cool_point) {
                weapon_cooldown++;
            }

            if (weapons_enabled && !is_disabled(is_player) && !is_cloaked() && !is_warping() &&
                weapon_cooldown >= cool_point) {
        ...
        if (has_active()) {
            uint32_t cool_point = get_cooldown(Game_Data::get_upgrade_type(get_active_name())->cooldown) *
                                  Engine::UPDATE_RATE / 1000;
        ...
        if (has_point_defense()) {
            uint32_t cool_point = get_cooldown(Game_Data::get_upgrade_type(get_point_defense_name())->cooldown) *
                                  Engine::UPDATE_RATE / 1000;
...
bool Ship::faction_is_valid (string faction, bool weapon_check) const {
    if (get_faction() == "player") {
        ...
        if (faction == "pirate" ||
            (faction == "player" && !Game::is_player_tractored() &&
...
int32_t Ship::get_nearest_valid_target_ship (const Quadtree<double, uint32_t>& quadtree_ships, uint32_t own_index,
                                             const Collision_Rect<double>& box_targeting, bool weapon_check) {
    ...
    for (size_t i = 0; i < nearby_ships.size(); i++) {
        if (!collisions.count(nearby_ships[i])) {
            ...
            if (ship.is_alive() && own_index != nearby_ships[i] && !ship.is_landing() &&
        ...
        if (nearest_index >= 0) {
            ...
            for (uint32_t i = 0; i < upgrade->shots; i++) {
                ...
                if (damage_type == "solid") {
                    ...
                    if (is_player) {
                        Screen_Shake::add_shake(Game_Constants::SHAKE_MAGNITUDE_WEAPON_SOLID,
                                                Game_Constants::SHAKE_LENGTH_WEAPON_SOLID);
                        Controller_Manager::make_rumble(Controller_Manager::CONTROLLER_ID_ALL,
                                                        Game_Constants::SHAKE_MAGNITUDE_WEAPON_SOLID,
                                                        Game_Constants::SHAKE_LENGTH_WEAPON_SOLID);
                    }
                    ...
                    if (is_player) {
                        Screen_Shake::add_shake(Game_Constants::SHAKE_MAGNITUDE_WEAPON_EXPLOSIVE,
                                                Game_Constants::SHAKE_LENGTH_WEAPON_EXPLOSIVE);
                        Controller_Manager::make_rumble(Controller_Manager::CONTROLLER_ID_ALL,
                                                        Game_Constants::SHAKE_MAGNITUDE_WEAPON_EXPLOSIVE,
                                                        Game_Constants::SHAKE_LENGTH_WEAPON_EXPLOSIVE);
                    }
                    ...
                    if (is_player) {
                        Screen_Shake::add_shake(Game_Constants::SHAKE_MAGNITUDE_WEAPON_ENERGY,
                                                Game_Constants::SHAKE_LENGTH_WEAPON_ENERGY);
                        Controller_Manager::make_rumble(Controller_Manager::CONTROLLER_ID_ALL,
                                                        Game_Constants::SHAKE_MAGNITUDE_WEAPON_ENERGY,
                                                        Game_Constants::SHAKE_LENGTH_WEAPON_ENERGY);
                    }
...
bool Ship::can_use_active (bool is_player) const {
    if (has_active() && !is_disabled(is_player)) {
        ...
        if (active_cooldown >= cool_point) {
            if (!is_player || !does_active_need_power(active_name) ||
                upgrade->power_use * Engine::UPDATE_RATE <= Game::get_power()) {
                return true;
            }
        }
    }
...
int32_t Ship::get_nearest_valid_target_shot (const Quadtree<double, uint32_t>& quadtree_shots,
...
void Ship::ai (const Quadtree<double, uint32_t>& quadtree_ships, const Quadtree<double, uint32_t>& quadtree_planets,
               const Quadtree<double, uint32_t>& quadtree_debris, uint32_t frame, uint32_t own_index, RNG& rng) {
    if (is_alive() && !is_landing() && !is_disabled(false) && is_in_processing_range()) {
        ...
        if (ai_has_proximity_target) {
            ...
            if (get_faction() == "police" && ai_proximity_target_is_player() && Game::notoriety_tier_1() &&
                get_distance_to_player() <= Game_Constants::TRACTOR_RANGE) {
                Game::tractor_player(angle + 180.0, own_index);

                set_braking(true);
            } else {
                if (ai_proximity_target_flee ||
                    get_distance_to_proximity_target() > Game_Constants::AI_FOLLOW_DISTANCE) {
                    thrusting = true;
                }
            }
        } else {
        ...
        if (get_faction() != "police" ||
            (!ai_proximity_target_is_player() &&
             (!Game::is_player_tractored() || Game::get_tractoring_ship_index() != own_index))) {
            use_active(false);
        }
    }
}
...
void Ship::rotation (bool is_player) {
    if (is_alive() && !is_landing() && (!is_player || !Game::player_is_landed()) && is_in_processing_range() &&
        get_angular_velocity().magnitude > 0.0) {
...
void Ship::movement (uint32_t own_index, const Quadtree<double, uint32_t>& quadtree_debris, const Quadtree<double,
    ...
    if (is_alive() && !is_landing() && (!is_player || !Game::player_is_landed()) && is_in_processing_range()) {
        ...
        if (!ai_ignore_angle_mods) {
            if (Math::abs(box.x - last_x) + Math::abs(box.y - last_y) <
                Game_Constants::AI_TIME_WITHOUT_MOVING_DISTANCE_THRESHOLD) {
                time_without_moving++;
            } else {
                time_without_moving = 0;
            }
                ...
                if (is_alive()) {
                    if (!collisions.count(nearby_debris[i])) {
                        ...
                        if (rng.random_range(0,
                                             99) < Game_Constants::COLLISION_CHANCE_DEBRIS &&
                            Collision::check_vertices_rect(get_collision_vertices(), vertices_collision)) {
            ...
            if (is_player) {
                ...
                for (size_t i = 0; i < nearby_items.size(); i++) {
                    if (!collisions.count(nearby_items[i])) {
                        ...
                        if (item.is_alive() && can_use_item(item.get_item_type()) &&
                            Collision::check_rect(item.get_box(), box)) {
...
void Ship::render (bool tractoring, bool is_player) {
    if (is_alive()) {
        ...
        bool in_camera = Collision::check_rect_rotated(box_for_camera_check * Game_Manager::camera_zoom,
                                                       Game_Manager::camera, angle_to_use, 0.0);
        ...
        if (in_camera) {
            if (is_player || !is_cloaked()) {
                ...
                sprite.render(box.x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                              box.y * Game_Manager::camera_zoom - Game_Manager::camera.y, opacity, scale, scale,
                              angle_to_use);
            }

            if (Game_Options::show_collision_outlines) {
                Collision_Rect<double> dodge_box(box.x - Game_Constants::DODGE_RANGE,
        ...
        if (tractoring) {
            ...
            tractor_sprite.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                  y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0, 1.0, 1.0, angle);
        }

        if (in_camera && get_faction() == "police" && ai_proximity_target_is_player() && !is_disabled(false) &&
            (Game::notoriety_tier_1() || Game::notoriety_tier_2())) {
            ...
            police_lights_sprite.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                        y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0, 1.0, 1.0,
                                        light_angle);
        }
        ...
        if (in_camera && scanner_startup_sprite.animating) {
            ...
            scanner_startup_sprite.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                          y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0, 1.0, 1.0,
                                          angle_to_use);
        }
    }
