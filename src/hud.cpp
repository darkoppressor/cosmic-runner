...
void Hud::render_bar (string font_name, string font_color, string bar_color, const Coords<double>& position, double a,
                      double b, bool show_background) {
    Bitmap_Font* font = 0;
    ...
    Render::render_rectangle(position.x + Game_Constants::HUD_BAR_OFFSET_X,
                             position.y + Game_Constants::HUD_BAR_OFFSET_Y, bar_width, Game_Constants::HUD_BAR_HEIGHT,
                             1.0, bar_color);
...
void Hud::render () {
    ...
    if (player.has_weapon()) {
        ...
        render_bar("", "ui_white", "hud_cooldown", position,
                   (double) cool_point - (double) player.get_weapon_cooldown(), (double) cool_point, false);
    }
    ...
    if (player.has_active()) {
        ...
        render_bar("", "ui_white", "hud_cooldown", position,
                   (double) cool_point - (double) player.get_active_cooldown(), (double) cool_point, false);
    }
}
